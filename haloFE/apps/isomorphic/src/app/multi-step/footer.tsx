...
export default function Footer({ isLoading, className }: FooterProps) {
  ...
  return (
      <footer
        className={cn(
          'fixed bottom-0 left-0 right-0 z-50 flex items-center justify-center gap-4 px-4 py-5 lg:px-8 4xl:px-10',
          expandedLeft ? 'xl:left-[398px]' : 'xl:left-[104px]',
          className
        )}
      >
      ...
      {step > 0 && step < 7 && (
        <Button
          ...
          variant="outline"
          onClick={gotoPrevStep}
          className="gap-1 border-white text-white bg-slate-400 hover:bg-blue hover:text-black"
        >
          <PiArrowUpLight className="-rotate-90" />
          Back
        </Button>
