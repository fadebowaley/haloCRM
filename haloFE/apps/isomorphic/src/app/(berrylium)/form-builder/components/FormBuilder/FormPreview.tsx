...
import { AiOutlineCalendar as CalendarIcon, AiOutlineClockCircle as ClockIcon } from 'react-icons/ai';
      ...
      case 'radio':
        return (
          <div key={id} className="space-y-2">
            ...
            <RadioGroup
              defaultValue={defaultValue}
              onValueChange={(value: any) => handleInputChange(id, value)}
            >
              {options && options.map((option, i) => (
                <div key={`${id}-option-${i}`} className="flex items-center space-x-2">
      ...
      case 'dropdown':
        return (
          <div key={id} className="space-y-2">
            ...
            <Select defaultValue={defaultValue} onValueChange={(value: any) => handleInputChange(id, value)}>
              <SelectTrigger id={id}>
            ...
            <Popover>
              <PopoverTrigger asChild>
                ...
                >
                  ...
                  {date[id] ? dayjs(date[id] as Date).format("MMM D, YYYY") : placeholder || "Pick a date"}
                </Button>
              </PopoverTrigger>
              <PopoverContent className="w-auto p-0" align="start">
                <Calendar
                  ...
                  onChange={(newDate: Date | null) => {
                    setDate(prev => ({ ...prev, [id]: newDate ?? undefined }));
        ...
        return (
          <div key={id} className="space-y-2">
            <div className="flex items-center space-x-2">
              <Switch 
                id={id}
                onCheckedChange={(checked: any) => handleInputChange(id, checked)}
              />
              <Label htmlFor={id}>
                {label} {properties.validation?.required && <span className="text-destructive">*</span>}
              </Label>
      ...
      case 'slider':
        return (
          <div key={id} className="space-y-2">
            ...
            <Slider
              id={id}
              defaultValue={[parseInt(defaultValue ?? "0") || min || 0]}
              min={min || 0}
              max={max || 100}
              step={step || 1}
              onValueChange={([value]: number[]) => handleInputChange(id, value)}
            />
            {helpText && <p className="text-sm text-muted-foreground">{helpText}</p>}
          </div>
        );
