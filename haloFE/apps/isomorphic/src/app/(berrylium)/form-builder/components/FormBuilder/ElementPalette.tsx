...
import { AiOutlineRight, AiOutlineDown } from 'react-icons/ai'; // Replace lucide-react icons with react-icons equivalents
            ...
            return (
              <Collapsible key={category} open={openCategories[category]} onOpenChange={() => toggleCategory(category)}>
                <CollapsibleTrigger className="flex items-center w-full justify-between text-sm font-medium p-1 hover:bg-muted/30 rounded">
                  <span>{category}</span>
                  {openCategories[category] ? <AiOutlineDown size={16} /> : <AiOutlineRight size={16} />}
                </CollapsibleTrigger>
